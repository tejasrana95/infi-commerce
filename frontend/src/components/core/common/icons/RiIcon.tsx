...
export default function RiIcon({ name, size = 24, ...props }: { name: string; size?: number;[key: string]: any }) {
    if (!name) return null;
    ...
    if (!iconCache.has(name)) {
        ...
        const IconComponent = dynamic(
            () => import('react-icons/ri').then((mod) => {
                const Icon = (mod as any)[name];
                if (!Icon) {
                    console.warn(`Icon "${name}" not found in react-icons/ri`);
                    return () => null;
                }
                return Icon;
            }),
