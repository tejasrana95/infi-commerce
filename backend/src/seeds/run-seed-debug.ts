...
async function runSeed() {
    ...
    try {
        ...
        if (config.env === 'production') {
            console.log('⚠️  Running in PRODUCTION mode. Existing data will NOT be deleted.');
        }
        ...
        for (const collectionName of importOrder) {
            ...
            if (collectionName === 'users') {
                console.log(`🔍 DEBUG: Processing 'users' collection`);
            ...
            try {
                ...
                if (error.code === 11000) {
                    console.log(`  ⚠️  ${collectionName}: Some duplicates skipped (${data.length} items processed)`);
                } else {
